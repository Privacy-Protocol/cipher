...
import { keccak256 as keccak2562, stringToHex as stringToHex2 } from "viem";
var CIPHER_ACTION_TYPES = {
  membership: keccak2562(stringToHex2("CIPHER_ACTION:CREDENTIAL_GATE:v1")),
  voting: keccak2562(stringToHex2("CIPHER_ACTION:PRIVATE_VOTE:v1"))
};
var CIPHER_VERIFIER_IDS = {
  membership: keccak2562(stringToHex2("CIPHER_VERIFIER:CREDENTIAL_GATE_HONK:v1")),
  voting: keccak2562(stringToHex2("CIPHER_VERIFIER:VOTING_HONK:v1"))
};
...
import { keccak256 as keccak2563, toHex } from "viem";
...
var NoopCipherPayloadStorageProvider = class {
  async put(params) {
    const ref = keccak2563(params.encrypted.ciphertext);
...
import { createPublicClient, http, isHex as isHex2, keccak256 as keccak2564, stringToHex as stringToHex3 } from "viem";
function asBytes32AppId(value) {
  ...
  return keccak2564(stringToHex3(`CIPHER_APP:${value}`));
}
...
var MembershipModule = class {
  ...
  async getContextState(contextId) {
    ...
    try {
      ...
      const msg = error instanceof BaseError2 ? error.shortMessage : "Failed to read membership context";
      throw new CipherError("TRANSACTION_ERROR", msg, error);
    }
  }
...
var VotingModule = class {
  ...
  async getProposalState(contextId) {
    ...
    try {
      ...
      const msg = error instanceof BaseError3 ? error.shortMessage : "Failed to read voting state";
      throw new CipherError("TRANSACTION_ERROR", msg, error);
    }
  }
  async isRootAllowed(contextId, root) {
...
//# sourceMappingURL=chunk-BTXVJWZR.js.map