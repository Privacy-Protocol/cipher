...
__export(react_exports, {
  ...
  useCipherClient: () => useCipherClient,
  useCipherContextClient: () => useCipherContextClient,
  useCipherRouter: () => useCipherRouter,
  useConfidentialGate: () => useConfidentialGate,
  useCredentialGateState: () => useCredentialGateState,
  useGenerateProof: () => useGenerateProof,
  useMembership: () => useMembership,
  useMembershipState: () => useMembershipState,
  usePrivateVote: () => usePrivateVote,
  useSubmitCipherAction: () => useSubmitCipherAction,
  useVoting: () => useVoting,
  useVotingState: () => useVotingState
});
...
var CipherContext = (0, import_react.createContext)(null);
function CipherProvider(props) {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(CipherContext.Provider, { value, children: props.children });
}
function useCipherContextClient() {
  const client = (0, import_react.useContext)(CipherContext);
  if (!client) {
    throw new CipherError(
      "TRANSACTION_ERROR",
...
function useCipherClient(options) {
  const contextClient = (0, import_react2.useContext)(CipherContext);
  const chain = options?.chain;
  const walletClient = options?.walletClient;
  const appId = options?.appId;
  const adapter = options?.adapter;
  const localClient = (0, import_react2.useMemo)(() => {
...
function useAsyncTask(fn) {
  const [state, setState] = (0, import_react3.useState)(() => createIdleAsyncState());
  const mountedRef = (0, import_react3.useRef)(true);
  const fnRef = (0, import_react3.useRef)(fn);
...
function useResolvedCipherClient(client) {
  const contextClient = (0, import_react4.useContext)(CipherContext);
...
function useCipherRouter(options = {}) {
  ...
  const refreshPointers = (0, import_react5.useCallback)(() => pointersTask.run(), [pointersTask.run]);
...
function useMembership(options = {}) {
  ...
  const [prepared, setPrepared] = (0, import_react6.useState)(null);
  ...
  const buildProofInput = (0, import_react6.useCallback)(
    (input) => client.membership.buildProofInput(input),
    [client]
  );
  const prepare = (0, import_react6.useCallback)(
    async (input) => {
      setPhase("preparing");
      setError(null);
      try {
  ...
  const submitPrepared = (0, import_react6.useCallback)(
    async (nextPrepared) => {
      setPhase("submitting");
      setError(null);
      setPrepared(nextPrepared);
  ...
  const computeExpectedRoot = (0, import_react6.useCallback)(
    (input) => client.membership.computeExpectedRoot(input),
    [client]
  );
  const computeExpectedNullifier = (0, import_react6.useCallback)(
...
function useMembershipState(options = {}) {
  ...
  const refreshContextState = (0, import_react7.useCallback)(
    (contextId) => {
  ...
  const checkRootAllowed = (0, import_react7.useCallback)(
    (params) => {
...
function useVoting(options = {}) {
  const client = useResolvedCipherClient(options.client);
  const [phase, setPhase] = (0, import_react8.useState)("idle");
  const [prepared, setPrepared] = (0, import_react8.useState)(null);
  ...
  const buildProofInput = (0, import_react8.useCallback)((input) => client.voting.buildProofInput(input), [client]);
  const prepare = (0, import_react8.useCallback)(
    async (input) => {
      setPhase("preparing");
      setError(null);
      try {
  ...
  const submitPrepared = (0, import_react8.useCallback)(
    async (nextPrepared) => {
      setPhase("submitting");
      setError(null);
      setPrepared(nextPrepared);
  ...
  const computeExpectedRoot = (0, import_react8.useCallback)(
    (input) => client.voting.computeExpectedRoot(input),
    [client]
  );
  const computeExpectedNullifier = (0, import_react8.useCallback)(
...
function useVotingState(options = {}) {
  ...
  const refreshProposalState = (0, import_react9.useCallback)(
    (contextId) => {
      const target = contextId ?? options.contextId;
      if (!target) {
  ...
  const checkRootAllowed = (0, import_react9.useCallback)(
    (params) => {
      const target = params.contextId ?? options.contextId;
      if (!target) {
  ...
  const getVote = (0, import_react9.useCallback)((actionId) => voteTask.run(actionId), [voteTask.run]);
